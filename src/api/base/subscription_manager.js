...
import { cloneObject, getPropertyValue, isDeepEqual, isEmptyObject } from '../../utils/object';
...
const SubscriptionManager = (() => {
    ...
    const dispatch = (response, sub_id) => {
        const stream_id =
            getPropertyValue(response, [response.msg_type, 'id']) || getPropertyValue(response.subscription, ['id']);
    ...
    const forget = (msg_type, fncCallback, match_values) => {
        if (typeof fncCallback !== 'function') {
            throw new Error(
                `Missing callback function. To forget all subscriptions of msg_type: ${msg_type}, please call forgetAll().`
            );
        }

        // find corresponding id(s)
        const sub_ids = Object.keys(subscriptions).filter(
            id => subscriptions[id].msg_type === msg_type && hasCallbackFunction(id, fncCallback)
        );
    ...
    const forgetAll = (...msg_types) => {
        ...
        return Promise.resolve(
            !isEmptyObject(types_to_forget) ? BinarySocket.send({ forget_all: Object.keys(types_to_forget) }) : {}
        );
    };
    ...
    const forgetStream = stream_id => {
        ...
        return Promise.resolve(
            stream_id
                ? BinarySocket.send({ forget: stream_id }).then(() => {
