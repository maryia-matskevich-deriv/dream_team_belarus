...
class Trader extends Common {
    ...
    async assertContractDetails() {
        ...
        if (!['over_under', 'even_odd', 'match_diff'].includes(this.contract)) {
            // Entry Spot check
            const entry_spot_tick =
                last_proposal_open_contract_message.proposal_open_contract.audit_details.all_ticks.find(
                    t => t.name === 'Entry Spot'
                );
        ...
        assert.equal(
            exit_time_displayed,
            `${end_time.getUTCFullYear()}-${(end_time.getUTCMonth() + 1).toString().padStart(2, '0')}-${end_time
                .getUTCDate()
                .toString()
                .padStart(2, '0')} ${end_time.getUTCHours().toString().padStart(2, '0')}:${end_time
                .getUTCMinutes()
                .toString()
