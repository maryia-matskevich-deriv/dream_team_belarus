...
class logger {
    ...
    static save = (test_path, name, results) => {
        ...
        if (!fs.existsSync(file_path)) fs.writeFileSync(file_path, JSON.stringify([{ name, results }]));
        else {
    ...
    static log = () => {
        const printLine = body => {
            console.log(Colors.FgYellow, body, Colors.RESET);
        };

        const printTable = body => {
            console.table(body);
        };

        const createReport = ([key, value]) => {
            console.log('>', Colors.FgYellow, key, Colors.RESET);
            if (['string', 'number'].includes(typeof value)) {
                printLine(value);
            } else if (value.length) {
                printTable(value);
            } else if (typeof value === 'object') {
                createReport(Object.entries(value));
            } else {
        ...
        files.forEach(file => {
            ...
            data.forEach(item => {
                ...
                Object.entries(item.results).forEach(createReport);
